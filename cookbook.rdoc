...
Once you are done with an image, you can either let Ruby's garbage collector
free its associated memory or call the FreeImage::Bitmap#free method.

Alternatively, you can pass a block to the open method similary to File.open. The
semantics are also the same, once you the block completes, the image will be
automaically freed for you.
